...
export PATH="/home/isac/Code/miniconda3/bin:$PATH"
root="/kyber/Data/Nanopore/projects/nanonome/analysis"
scr="/home/isac/Code/nanoNOMe/scripts/methylation_accessibility_distance.py"
ref=/mithril/Data/NGS/Reference/hg38_noalt/hg38_noalt.fa
outdir=$root/plots/mdist

cpg="$root/data/nanonome/pooled/mbed/GM12878_nanoNOMe.pooled.cpg.meth.bed.gz"
gpc="$root/data/nanonome/pooled/mbed/GM12878_nanoNOMe.pooled.gpc.meth.bed.gz"

## high
#reg="$root/data/gm12878/GM12878_highexpression_genes.TSS.2000bp.bed"
#regname="highexpression_promoters"
#cell="GM12878"
#out=$outdir/$cell.methdistance.$regname.txt
#log=$outdir/$cell.methdistance.$regname.log
#shuf -n1000 $reg | $scr -v -c $cpg -g $gpc -f $ref -o $out 2> $log
## low
...
```{r plot,fig.width = 12,fig.height = 8,echo=FALSE,message=FALSE}
root="/kyber/Data/Nanopore/projects/nanonome/analysis"
...
for (regname in regnames){
    dfp=file.path(outdir,paste(cell,"methdistance",regname,"txt",sep="."))
    tb = read_tsv(dfp)
    tb.list[[regname]] = tb %>% mutate(regname=regname)
    tb = tb[which(tb$region=="all"),]
    
    g = ggplot(tb,aes(x=distance,y=count)) +
...
g = ggplot(tb.all,aes(x=distance,y=count,group=regname,color=regname)) +
  facet_wrap(~relationship,scales="free") +
